...
## Browser Release Evidence Gate

After the API release gate passes, run the browser gate from `apps/web`:

```powershell
npm.cmd run test:e2e:live:v26
```

The browser gate starts a real local FastAPI API and Vite app through Playwright, using an isolated SQLite database unless `DATABASE_URL` is explicitly supplied. It must pass browser-visible checks for:

- organizer login and event workspace;
- merchant login and interaction package visibility;
- merchant live-status reporting and organizer exception suggestion visibility;
- public H5 mobile scan, coupon claim, and coupon redemption;
- organizer review report visibility;
- forbidden internal terms absent from merchant and public surfaces.

The gate writes structured JSON evidence to:

- `docs/research/assets/v2.6-browser-release-evidence-gate/browser-release-gate-result.json`.

Screenshot PNG files generated by the browser gate are local inspection artifacts and are not committed by default. The JSON evidence records screenshot file names and hashes only.
