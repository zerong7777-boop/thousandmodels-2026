...
interface AgentSafetyChecklistProps {
  hasModelEvidence?: boolean;
}

export function AgentSafetyChecklist({ hasModelEvidence = false }: AgentSafetyChecklistProps) {
  const { t } = useI18n();
  return (
    <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        {!hasModelEvidence ? <Badge variant="neutral">{t("organizer.agentEvidence.noModelCall")}</Badge> : null}
      </div>
    </div>
  );
}
